...
@section('content')
    ...
    @livewire('personnel.children')
@endsection
@push('scripts')
    <script src="{{ asset('/extra-assets/html2canvas/html2canvas.min.js') }}"></script>
    <script>
        ...
        window.addEventListener('showChildModal', function() {
            ...
            $('#children_modal form').on('submit', function(e) {
                ...
                var trang_thai = Array.from(document.querySelector('#trang_thai')
                    .selectedOptions).map(
                    option => option.value
                );
